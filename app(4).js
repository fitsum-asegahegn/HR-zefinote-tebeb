...
// Pulls the first run of 3+ digits out of an address string and takes its
// first 3 digits as the neighborhood code — e.g. "ኮዬ 435/17" -> 435.
// Used to bucket members into 100-wide ranges (400 -> 400-499) since
// nearby house numbers roughly cluster by neighborhood.
// House numbers in this data are consistently written as "NNN/NN" (e.g.
// "435/17"). Some addresses also have a block/site prefix number before
// the word ቤት/ብ that is NOT the house code (e.g. "400ቤት 457/11" — the
// real code is 457, not 400), and a few have a smaller sub-city/woreda
// code before the real one (e.g. "10/90  262/07" — the real code is
// 262). So: only consider numbers sitting directly against a slash (the
// house-code format), and when more than one appears, prefer the one
// with more digits, since real house codes run 3 digits while prefix
// codes tend to be 1-2. Falls back to any bare digit run only when the
// address has no slash pattern at all (e.g. a plain "116").
function extractAddressCode(address) {
  if (!address) return null;
  const str = String(address);
  const slashMatches = [...str.matchAll(/(\d+)\s*\//g)];
  if (slashMatches.length) {
    slashMatches.sort((a, b) => b[1].length - a[1].length);
    return parseInt(slashMatches[0][1].slice(0, 3), 10);
  }
  const match = str.match(/\d{2,}/);
  if (!match) return null;
  return parseInt(match[0].slice(0, 3), 10);
}
...
// Center-crops to the target aspect ratio (matching the printed ID card's
// photo box) and re-encodes as compressed JPEG, so a photo taken straight
// from a phone camera doesn't end up as several MB sitting in IndexedDB
// and getting pushed through Supabase sync on every edit.
function resizeImageFile(file, maxW = 300, maxH = 400) {
  return new Promise((resolve, reject) => {
...
async function renderDashboard() {
  ...
  const hrLookaheadCutoff = isoDate(addDays(new Date(), 15));
  const hrDue = hrDueAll.filter((e) => e.nextDate && e.nextDate <= hrLookaheadCutoff);
  const members = await getAll("members");
  const attendance = await getAll("attendance");
  const today = todayISO();
  ...
  function draw(list) {
    ...
    el("memberList").innerHTML = list.map((m) => `
      <div class="list-row">
        ...
        ${m.photo
          ? `<img src="${m.photo}" style="width:36px;height:36px;border-radius:8px;object-fit:cover;flex:0 0 auto;">`
          : `<div style="width:36px;height:36px;border-radius:8px;background:var(--bg2);border:1px solid var(--border);color:var(--muted);display:flex;align-items:center;justify-content:center;font-size:0.85rem;flex:0 0 auto;">${(m.fullName || "?").trim().charAt(0).toUpperCase()}</div>`}
        <div style="flex:1;">
          <b>${m.fullName}</b><br>
          <span class="muted">
            ${m.phone || ""}
  ...
  function applyFilters() {
    ...
    if (areaBaseRaw !== "") {
      ...
      filtered = filtered.filter((m) => {
        const code = extractAddressCode(m.address);
        return code !== null && code >= base && code <= upper;
      });
      countEl.style.display = "";
      countEl.textContent = lang === "am"
        ? `${filtered.length} አባላት ከ${base}-${upper} አካባቢ ውስጥ`
        : `${filtered.length} member(s) in the ${base}-${upper} range`;
    } else {
...
window.showQr = async (id) => {
  ...
  box.innerHTML = `
    <div class="modal-inner">
      ...
      <div id="idCardPreview" style="display:flex;justify-content:center;"></div>
      <p class="muted" style="max-width:260px;">${t("members.softCopyNote")}</p>
  ...
  const canvas = await buildIdCardCanvas(m);
...
// download and share so both produce the same complete card, not just
// the bare QR code. ----------
...
function renderQrToDataUrl(text, size) {
  return new Promise((resolve) => {
...
async function buildIdCardCanvas(member) {
  const lang = getLang();
  const W = 900, H = 520;
  const canvas = document.createElement("canvas");
  ...
  const px = 40, py = 40, pw = 230, ph = 300;
  if (member.photo) {
    try {
      const photoImg = await loadImageEl(member.photo);
      ctx.save();
      ctx.beginPath();
      ...
      ctx.restore();
    } catch (e) { /* fall through to placeholder look below */ }
  }
  if (!member.photo) {
    ...
    ctx.fillText(lang === "am" ? "ፎቶ የለም" : "No Photo", px + pw / 2, py + ph / 2);
    ctx.textAlign = "left";
  }
  ...
  ctx.font = "bold 30px 'Noto Sans Ethiopic', sans-serif";
  wrapCanvasText(ctx, member.fullName, px, py + ph + 40, pw + 40, 34, 2);
  ctx.font = "16px 'Noto Sans Ethiopic', sans-serif";
  ctx.fillStyle = "#666666";
  const orgLine = (member.grade ? `${lang === "am" ? "ክፍል" : "Grade"} ${member.grade} · ` : "") + "ፍኖተ ጥበብ ሰ/ት/ቤት";
  ctx.fillText(orgLine, px, py + ph + 82);

  const qrDataUrl = await renderQrToDataUrl(member.qrId, 260);
  const qrImg = await loadImageEl(qrDataUrl);
  const qx = W - 300, qy = 50, qs = 260;
  ctx.drawImage(qrImg, qx, qy, qs, qs);
  ctx.font = "14px 'JetBrains Mono', monospace";
  ctx.fillStyle = "#999999";
  ctx.fillText(member.qrId, qx, qy + qs + 26);

  return canvas;
}

function downloadIdCard(canvas, name) {
  canvas.toBlob((blob) => {
    if (blob) downloadBlob(blob, `id-${name.replace(/\s+/g, "_")}.png`);
  }, "image/png");
}

async function shareIdCard(canvas, name) {
  canvas.toBlob(async (blob) => {
    if (!blob) return;
    const file = new File([blob], `id-${name.replace(/\s+/g, "_")}.png`, { type: "image/png" });
...
window.openRegistrationModal = async function(editId) {
  ...
  box.innerHTML = `
    <div class="modal-inner" style="max-width:500px;max-height:90vh;overflow-y:auto;text-align:left;">
      ...
      <form id="regForm">
        ...
        <label>${getLang() === "am" ? "ፎቶ" : "Photo"}</label>
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
          <img id="f_photoPreview" src="${member?.photo || ""}" style="width:64px;height:85px;object-fit:cover;border-radius:6px;border:1px solid var(--border);background:var(--bg2);${member?.photo ? "" : "display:none;"}">
          <div id="f_photoPlaceholder" style="width:64px;height:85px;border-radius:6px;border:1px dashed var(--border);display:${member?.photo ? "none" : "flex"};align-items:center;justify-content:center;color:var(--muted);font-size:0.65rem;text-align:center;">${getLang() === "am" ? "ምንም" : "None"}</div>
          <div>
            <label class="btn-secondary file-btn" style="display:inline-flex;">${getLang() === "am" ? "ፎቶ ምረጥ" : "Choose Photo"}<input type="file" id="f_photoInput" accept="image/*" style="display:none;"></label>
            <button type="button" class="btn-small" id="f_photoRemove" style="margin-left:6px;${member?.photo ? "" : "display:none;"}">${getLang() === "am" ? "አስወግድ" : "Remove"}</button>
          </div>
        </div>
  ...
  el("f_photoInput").onchange = async (e) => {
    ...
    if (!file) return;
    try {
      photoDataUrl = await resizeImageFile(file, 300, 400);
      el("f_photoPreview").src = photoDataUrl;
      el("f_photoPreview").style.display = "";
      el("f_photoPlaceholder").style.display = "none";
      el("f_photoRemove").style.display = "";
    } catch (err) {
      await showAlert(getLang() === "am" ? "ፎቶውን ማንበብ አልተቻለም" : "Couldn't read that photo.");
    }
  };
  el("f_photoRemove").onclick = () => {
    photoDataUrl = null;
    el("f_photoInput").value = "";
    el("f_photoPreview").style.display = "none";
    el("f_photoPlaceholder").style.display = "flex";
    el("f_photoRemove").style.display = "none";
  };
...
// Marking someone here just sets member.active = false — the same flag
// computeConsecutiveAbsences/computeProgramSpecificAbsences/
// computeConfessionDue/computeGradeStats already filter on, so it takes
// effect everywhere calls/reminders are computed with no other changes
// needed. They still show up in the Members tab and can be re-activated
// here at any time.
async function renderLostMembers() {
  const lang = getLang();
  const members = (await getAll("members")).sort((a, b) => a.fullName.localeCompare(b.fullName));
  const pending = new Map(members.map((m) => [m.id, m.active === false]));

  el("view").innerHTML = `
    ${groupsBackBtn()}
    <h3 class="section-title">${lang === "am" ? "የጠፉ አባላት" : "Lost Members"}</h3>
    ...
    <div class="toolbar" style="margin-top:12px;">
      <button id="lostSaveBtn" class="btn-primary">${t("settings.save")}</button>
    </div>
  `;
  ...
  function draw(list) {
    el("lostList").innerHTML = list.map((m) => `
      <div class="list-row">
        ...
        <div style="flex:1;">
          <b>${m.fullName}</b><br>
          <span class="muted">${m.phone || ""}${m.grade ? " · " + t("members.gradeShort", { n: m.grade }) : ""}${m.address ? " · " + m.address : ""}</span>
        </div>
        ${pending.get(m.id) ? `<span class="badge badge-red">${lang === "am" ? "የጠፋ" : "Lost"}</span>` : ""}
      </div>`).join("");
    el("lostList").querySelectorAll('input[type="checkbox"]').forEach((cb) => {
      cb.onchange = () => {
        pending.set(cb.dataset.id, cb.checked);
        draw(currentList);
      };
    });
  }
  ...
  draw(currentList);
  el("lostSearch").oninput = (e) => {
    const q = e.target.value.toLowerCase();
    currentList = members.filter((m) => m.fullName.toLowerCase().includes(q));
    draw(currentList);
  };

  el("lostSaveBtn").onclick = async () => {
    let changedCount = 0;
    for (const m of members) {
      const shouldBeLost = pending.get(m.id);
      const currentlyLost = m.active === false;
      if (shouldBeLost !== currentlyLost) {
        m.active = !shouldBeLost;
        m.synced = false;
        await put("members", m);
        changedCount++;
      }

...
// ---------- Stubs for missing dependencies (replace with real implementations) ----------
// Translation / Language
let _lang = 'en'; // default
function getLang() { return _lang; }
function setLang(lang) { _lang = lang; }
const translations = {
  en: {
    'app.title': 'Finote Tsebeb',
    'app.subtitle': 'Digital Attendance',
    'app.motto': 'እውነት ያውቃችኋል',
    ...
    // ... (add all keys used, or use a fallback)
  },
  am: {
    // similar
  }
...
function t(key, params) {
  let str = (translations[getLang()] && translations[getLang()][key]) || key;
  if (params) {
...
// Supabase / Cloud
let sbClient = null;
function skipCloudFlag() { return true; } // always offline mode
function initSupabaseClient() { sbClient = null; }
function getSession() { return null; }
function mirrorAuthForSW(session) {}
function fetchUserRole(session) { window.currentUserRole = 'admin'; }
function fetchDisplayName(session) { window.currentDisplayName = 'Device'; }
function bioIsEnabled() { return false; }
function unlockWithBiometric() { return Promise.resolve(true); }
function getSupabaseConfig() { return null; }
function syncNow() { console.log('Sync called (stub)'); }
function checkAndNotify() { console.log('Check notifications (stub)'); }
...
const ETH_MONTH_NAMES_AM = ["መስከረም", "ጥቅምት", "ኅዳር", "ታኅሣሥ", "ጥር", "የካቲት", "መጋቢት", "ሚያዝያ", "ግንቦት", "ሰኔ", "ሐምሌ", "ነሐሴ", "ጳጉሜ"];
const ETH_MONTH_NAMES_EN = ["Meskerem", "Tikimt", "Hidar", "Tahsas", "Tir", "Yekatit", "Megabit", "Miyazya", "Ginbot", "Sene", "Hamle", "Nehase", "Pagume"];
...
// ---------- Ethiopian Calendar Helpers (Fixed) ----------
function gregorianToEthiopian(date) {
  // Accurate conversion using known offset and month lengths
  ...
  // Ethiopian epoch JDN for 1 Meskerem 1 EC
  const ethiopianEpochJDN = 1724226; // 1 Meskerem 1 EC
  const daysSinceEpoch = jdn - ethiopianEpochJDN;
  if (daysSinceEpoch < 0) {
    // Before epoch, return year 0? But we'll handle by returning a default.
    return { year: 0, month: 0, day: 1 };
  }

  let ethYear = 1;
  let remaining = daysSinceEpoch;
  while (remaining >= (isEthiopianLeapYear(ethYear) ? 366 : 365)) {
    remaining -= isEthiopianLeapYear(ethYear) ? 366 : 365;
    ethYear++;
  }
  // Now remaining is the day of the year (0-based)
  const monthLengths = [30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 5]; // 13 months, last 5 or 6 days
  if (isEthiopianLeapYear(ethYear)) monthLengths[12] = 6;
  let ethMonth = 0;
  while (remaining >= monthLengths[ethMonth]) {
    remaining -= monthLengths[ethMonth];
    ethMonth++;
  }
  const ethDay = remaining + 1;
  return { year: ethYear, month: ethMonth, day: ethDay };
}

function isEthiopianLeapYear(year) {
  // Ethiopian leap year occurs every 4 years (no century exception)
  return year % 4 === 0;
}

function ethLabel(isoDate) {
  try {
    const d = new Date(isoDate + 'T00:00:00');
    const eth = gregorianToEthiopian(d);
    const monthNames = getLang() === "am" ? ETH_MONTH_NAMES_AM : ETH_MONTH_NAMES_EN;
    return `${eth.day} ${monthNames[eth.month]} ${eth.year}`;
  } catch (e) {
    return isoDate;
  }
}

function computeEthAwareNextDate(timingText, fromDate = new Date()) {
  const today = new Date(fromDate);
  const lower = timingText.toLowerCase();
  if (/ወርሃዊ|monthly|ወር|month/.test(lower)) {
    return isoDate(addMonths(today, 1));
  }
  if (/ሩብ ዓመት|quarterly|quarter/.test(lower)) {
    return isoDate(addMonths(today, 3));
  }
  if (/2 ጊዜ|twice|ግማሽ ዓመት|semiannual|bi-annual|182/.test(lower)) {
    return isoDate(addMonths(today, 6));
  }
  if (/እንደአስፈላጊነቱ|እንደአጋጣሚው|as needed|as-needed|እንደተቀላቀሉ|እንደአስፈላጊ/.test(lower)) {
    return null; // No fixed recurrence
  }
  // Default: annual
  return isoDate(addMonths(today, 12));
}
...
function drawCharts(attendance, progs, gradeStats) {
  ...
  if (trendCanvas) {
    ...
    if (!sortedDates.length) {
      // FIX: previously used `return;` here, which exited the whole drawCharts()
      // function and silently skipped the program and grade charts below whenever
      // there was no attendance data yet. Now we just skip this chart only.
  ...
  if (progCanvas) {
    ...
    if (total === 0) {
      // FIX: same early-return bug as above — no longer bails out of drawCharts().
...
// ---- Additional stubs for functions that might be called from HTML ----
function renderSupabaseSetup() { el("view").innerHTML = `<div class="auth-wrap"><h2>Setup</h2><p>Supabase not configured.</p></div>`; }
function renderAuthScreen() { el("view").innerHTML = `<div class="auth-wrap"><h2>Login</h2><p>Please log in (stub).</p></div>`; }
function renderOfflineNoSession() { el("view").innerHTML = `<div class="auth-wrap"><h2>Offline</h2><p>No session and offline.</p></div>`; }
